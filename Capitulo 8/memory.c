...
{
    //esta dentro do heap
    if (((uint64 *)ptr) >= HEAP_START)
        if (((uint8 *)ptr) <= HEAP_END)
        {

            uint8 *desc;
            uint64 N_page;
            //divido ptr por PAGE_SIZE
            N_page = ((uint64)ptr - alloc_start) / PAGE_SIZE;

            //faço o deslocamento das paginas
            desc = (uint8 *)(N_page + (uint64)HEAP_START);

            //se a pagina nao estiver livre
            if (!free_page(*desc))
            {
                //enquanto for diferente da ultima pagina
